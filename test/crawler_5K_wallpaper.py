...
def Down_load(file_url, file_full_name, now_photo_count, all_photo_count):
    ...
    # 开始下载图片
    with contextlib.closing(requests.get(file_url, headers=headers, stream=True)) as response:
...
def crawler_photo(type_id, photo_count):
    ...
    print('respond content=', respond.content)
    ...
    if not os.path.exists('./' + str(type_id)):
        ...
        L = [x for x in os.listdir('./' + str(type_id))]
    ...
    for photo in photo_data:
        ...
        if L and file_name_only in L:
            print('{} is alread exist...'.format(file_name_only))
            continue
